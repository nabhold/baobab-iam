...
**Status:** Scoped — phases 1-2 complete, phase 3 in progress
**Date:** 2026-09-10
...
3. **`baobab-iam` — workload registry as source of provisioning** (gap 3, in progress): `scripts/bootstrap.sh` reads phase 2's registry file (vendored) to drive which `*-workload.json` clients exist/what scopes+audiences they get; expand `tests/integration/run.sh` with the remaining §102 isolation cases not yet covered (wrong-environment credential, cross-workload impersonation attempt).
